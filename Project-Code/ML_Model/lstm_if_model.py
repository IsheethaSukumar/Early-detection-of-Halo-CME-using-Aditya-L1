"""
BiLSTM + Isolation Forest Anomaly Score
========================================
Solar Halo CME Forecasting & Nowcasting  |  Aditya-L1

Identical to lstm_model.py EXCEPT the input CSV is
...
These flow into every 24-hour sliding window as additional channels,
so the LSTM can learn temporal anomaly patterns alongside solar wind.

Artifacts saved with "_if" suffix to co-exist alongside baseline.

Usage:
    python ML_Model/lstm_if_model.py
...
def load_data() -> pd.DataFrame:
    print("\n[1/5] Loading IF-augmented dataset ...")
...
def feature_engineering(df: pd.DataFrame):
    """
    anomaly_score and is_anomaly are NOT in DROP_COLS, so they are
    automatically included as extra feature channels in every window.
    """
    print("\n[2/5] Feature engineering  (anomaly_score + is_anomaly included) ...")
    feat_cols = [
    ...
    print(f"      Features : {X.shape[1]}  (baseline 57 + 2 IF features)")
    # Confirm IF features are present
    for c in ["anomaly_score", "is_anomaly"]:
        if c in feat_cols:
            print(f"        [OK] '{c}' included at index {feat_cols.index(c)}")
...
def build_sequences_and_split(X: pd.DataFrame, y: np.ndarray):
    """
    Builds sliding windows of WINDOW_SIZE hours.
    anomaly_score and is_anomaly flow into every window as extra channels --
    the LSTM can therefore learn temporal patterns in the anomaly signal.
    """
    ...
    X_seq = np.array([X_arr[i : i + WINDOW_SIZE] for i in range(n_seq)],
                     dtype=np.float32)
    y_seq = y[WINDOW_SIZE - 1 :].astype(np.float32)
    ...
    scaler_path = ARTIFACT_DIR / "lstm_if_scaler.pkl"
...
class HaloCME_BiLSTM_IF(nn.Module):
    ...
    Identical architecture to the baseline, but n_features is now 59
    (57 solar-wind + anomaly_score + is_anomaly).
...
def evaluate(model, test_loader, yw_te: np.ndarray):
    ...
    thr_path = ARTIFACT_DIR / "lstm_if_threshold.txt"
...
def _plot_roc_pr(y_test, y_prob, roc_auc: float, pr_auc: float) -> None:
    ...
    fig.suptitle("BiLSTM + IF Anomaly Score -- Halo CME -- Test Set",
    ...
    plt.tight_layout()
    out = ARTIFACT_DIR / "lstm_if_evaluation.png"
...
def _plot_training_curve(history: dict) -> None:
    ...
    fig.suptitle("BiLSTM + IF  --  Training Curve", fontsize=12, fontweight="bold")
    fig.tight_layout()
    out = ARTIFACT_DIR / "lstm_if_training_curve.png"
    plt.savefig(out, dpi=150)
    plt.close()
...
def print_comparison(roc_auc: float, pr_auc: float):
    BASELINE_ROC = None
    # Try to parse from lstm_res.txt
    lstm_res = RESULTS_DIR / "lstm_res.txt"
    if lstm_res.exists():
        for line in lstm_res.read_text(encoding="utf-8", errors="ignore").splitlines():
            if "ROC-AUC" in line and ":" in line:
                try:
                    BASELINE_ROC = float(line.split(":")[-1].strip().split()[0])
                    break
                except Exception:
                    pass

    if BASELINE_ROC is None:
        print("\n  [NOTE] Could not parse baseline ROC-AUC from lstm_res.txt -- skipping delta.")
        return

    delta = roc_auc - BASELINE_ROC
    sign  = "+" if delta >= 0 else ""
    print(f"\n  {'='*50}")
    print(f"  MODEL COMPARISON  (BiLSTM baseline vs BiLSTM + IF)")
    print(f"  {'='*50}")
    print(f"  {'Metric':<18} {'Baseline':>12} {'+ IF Score':>12} {'Delta':>8}")
    print(f"  {'-'*50}")
    print(f"  {'ROC-AUC':<18} {BASELINE_ROC:>12.4f} {roc_auc:>12.4f} {sign}{delta:>7.4f}")
    print(f"  {'='*50}")


# -- Save model ----------------------------------------------------------------
def save_model(model, n_features: int) -> None:
    print("\n[Save] Persisting BiLSTM+IF model ...")
    model_path = ARTIFACT_DIR / "lstm_if_model.pt"
    torch.save(model.state_dict(), model_path)
    print(f"  State dict -> {model_path}  ({model_path.stat().st_size // 1024:,} KB)")

    cfg = {
        "n_features": n_features, "hidden1": HIDDEN1, "hidden2": HIDDEN2,
        "dropout": DROPOUT, "window": WINDOW_SIZE,
        "note": "trained with anomaly_score + is_anomaly from IsolationForest",
    }
    cfg_path = ARTIFACT_DIR / "lstm_if_config.json"
...
if __name__ == "__main__":
    log_path  = RESULTS_DIR / "lstm_if_res.txt"
    _log_file = open(log_path, "w", encoding="utf-8")
    sys.stdout = _Tee(sys.__stdout__, _log_file)

    try:
        banner("SOLAR HALO CME  --  BiLSTM + Isolation Forest Score")
        print(f"  Device  : {DEVICE}")
        if DEVICE.type == "cuda":
            print(f"  GPU     : {torch.cuda.get_device_name(0)}")
        print(f"  Window  : {WINDOW_SIZE}h look-back")
        print(f"  Split   : stratified random 68/12/20 (sequence-level)")
        print(f"  Extra   : anomaly_score + is_anomaly in every window channel")
        ...
        print("    lstm_if_model.pt  |  lstm_if_config.json  |  lstm_if_scaler.pkl")
        print(f"    lstm_if_threshold.txt          (thr={thr:.4f})")
        print("    lstm_if_evaluation.png  |  lstm_if_training_curve.png")
